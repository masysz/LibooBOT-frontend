...
const Homepage: FunctionComponent<HomepageProps> = (): ReactElement => {
    ...
    const swapColorBasedOnStatus = () => {
        const totalPoints = (userProfileInformation?.points ?? 0) + localPoints;
        const status = metrics(totalPoints)?.status;
        switch (status) {
                    ...
                    <div className="flex flex-col items-center mb-12">
                        <div className="flex flex-row gap-2 items-center">
                            ...
                            <h1 className="text-[40px] text-white font-extrabold">
                                {(userProfileInformation.points ?? 0 + localPoints).toLocaleString()}
                                {metrics(localPoints + (userProfileInformation.points ?? 0))?.pointSuffix}
                            </h1>
                        </div>
                        <div className="flex flex-row gap-3 items-center">
                            <div className="flex flex-row gap-2 items-center">
                                ...
                                <p className={`${swapColorBasedOnStatus()} text-sm`}>
                                    {metrics(localPoints + (userProfileInformation.points ?? 0))?.status}
                                </p>
                            </div>
